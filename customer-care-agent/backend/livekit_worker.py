...
from backend.db import archive_shopping_list, get_user
...
async def send_greeting(
    ...
    add_message(user_id, "agent", greeting)
...
async def main():
    ...
    def on_join(participant: rtc.RemoteParticipant):
        ...
        user_info = get_user(participant.identity) or {}
        real_name = user_info.get("name", "Customer")
        
        asyncio.create_task(
            send_greeting(source, speaking, participant.identity, real_name)
        )
